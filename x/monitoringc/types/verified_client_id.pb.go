// Code generated by protoc-gen-gogo. DO NOT EDIT.
// source: network/monitoringc/v1/verified_client_id.proto

package types

import (
	fmt "fmt"
	proto "github.com/cosmos/gogoproto/proto"
	io "io"
	math "math"
	math_bits "math/bits"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.GoGoProtoPackageIsVersion3 // please upgrade the proto package

type VerifiedClientID struct {
	LaunchId     uint64   `protobuf:"varint,1,opt,name=launch_id,json=launchId,proto3" json:"launch_id,omitempty"`
	ClientIdList []string `protobuf:"bytes,2,rep,name=client_id_list,json=clientIdList,proto3" json:"client_id_list,omitempty"`
}

func (m *VerifiedClientID) Reset()         { *m = VerifiedClientID{} }
func (m *VerifiedClientID) String() string { return proto.CompactTextString(m) }
func (*VerifiedClientID) ProtoMessage()    {}
func (*VerifiedClientID) Descriptor() ([]byte, []int) {
	return fileDescriptor_f590a6fd2362c575, []int{0}
}
func (m *VerifiedClientID) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *VerifiedClientID) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_VerifiedClientID.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
			return nil, err
		}
		return b[:n], nil
	}
}
func (m *VerifiedClientID) XXX_Merge(src proto.Message) {
	xxx_messageInfo_VerifiedClientID.Merge(m, src)
}
func (m *VerifiedClientID) XXX_Size() int {
	return m.Size()
}
func (m *VerifiedClientID) XXX_DiscardUnknown() {
	xxx_messageInfo_VerifiedClientID.DiscardUnknown(m)
}

var xxx_messageInfo_VerifiedClientID proto.InternalMessageInfo

func (m *VerifiedClientID) GetLaunchId() uint64 {
	if m != nil {
		return m.LaunchId
	}
	return 0
}

func (m *VerifiedClientID) GetClientIdList() []string {
	if m != nil {
		return m.ClientIdList
	}
	return nil
}

func init() {
	proto.RegisterType((*VerifiedClientID)(nil), "network.monitoringc.v1.VerifiedClientID")
}

func init() {
	proto.RegisterFile("network/monitoringc/v1/verified_client_id.proto", fileDescriptor_f590a6fd2362c575)
}

var fileDescriptor_f590a6fd2362c575 = []byte{
	// 209 bytes of a gzipped FileDescriptorProto
	,
}

func (m *VerifiedClientID) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *VerifiedClientID) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *VerifiedClientID) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if len(m.ClientIdList) > 0 {
		for iNdEx := len(m.ClientIdList) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.ClientIdList[iNdEx])
			copy(dAtA[i:], m.ClientIdList[iNdEx])
			i = encodeVarintVerifiedClientId(dAtA, i, uint64(len(m.ClientIdList[iNdEx])))
			i--
			dAtA[i] = 0x12
		}
	}
	if m.LaunchId != 0 {
		i = encodeVarintVerifiedClientId(dAtA, i, uint64(m.LaunchId))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
}

func encodeVarintVerifiedClientId(dAtA []byte, offset int, v uint64) int {
	offset -= sovVerifiedClientId(v)
	base := offset
	for v >= 1<<7 {
		dAtA[offset] = uint8(v&0x7f | 0x80)
		v >>= 7
		offset++
	}
	dAtA[offset] = uint8(v)
	return base
}
func (m *VerifiedClientID) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	if m.LaunchId != 0 {
		n += 1 + sovVerifiedClientId(uint64(m.LaunchId))
	}
	if len(m.ClientIdList) > 0 {
		for _, s := range m.ClientIdList {
			l = len(s)
			n += 1 + l + sovVerifiedClientId(uint64(l))
		}
	}
	return n
}

func sovVerifiedClientId(x uint64) (n int) {
	return (math_bits.Len64(x|1) + 6) / 7
}
func sozVerifiedClientId(x uint64) (n int) {
	return sovVerifiedClientId(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *VerifiedClientID) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return ErrIntOverflowVerifiedClientId
			}
			if iNdEx >= l {
				return io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= uint64(b&0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: VerifiedClientID: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: VerifiedClientID: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field LaunchId", wireType)
			}
			m.LaunchId = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowVerifiedClientId
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				m.LaunchId |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ClientIdList", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowVerifiedClientId
				}
				if iNdEx >= l {
					return io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				stringLen |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			intStringLen := int(stringLen)
			if intStringLen < 0 {
				return ErrInvalidLengthVerifiedClientId
			}
			postIndex := iNdEx + intStringLen
			if postIndex < 0 {
				return ErrInvalidLengthVerifiedClientId
			}
			if postIndex > l {
				return io.ErrUnexpectedEOF
			}
			m.ClientIdList = append(m.ClientIdList, string(dAtA[iNdEx:postIndex]))
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipVerifiedClientId(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return ErrInvalidLengthVerifiedClientId
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
func skipVerifiedClientId(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	depth := 0
	for iNdEx < l {
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowVerifiedClientId
			}
			if iNdEx >= l {
				return 0, io.ErrUnexpectedEOF
			}
			b := dAtA[iNdEx]
			iNdEx++
			wire |= (uint64(b) & 0x7F) << shift
			if b < 0x80 {
				break
			}
		}
		wireType := int(wire & 0x7)
		switch wireType {
		case 0:
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return 0, ErrIntOverflowVerifiedClientId
				}
				if iNdEx >= l {
					return 0, io.ErrUnexpectedEOF
				}
				iNdEx++
				if dAtA[iNdEx-1] < 0x80 {
					break
				}
			}
		case 1:
			iNdEx += 8
		case 2:
			var length int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return 0, ErrIntOverflowVerifiedClientId
				}
				if iNdEx >= l {
					return 0, io.ErrUnexpectedEOF
				}
				b := dAtA[iNdEx]
				iNdEx++
				length |= (int(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
			}
			if length < 0 {
				return 0, ErrInvalidLengthVerifiedClientId
			}
			iNdEx += length
		case 3:
			depth++
		case 4:
			if depth == 0 {
				return 0, ErrUnexpectedEndOfGroupVerifiedClientId
			}
			depth--
		case 5:
			iNdEx += 4
		default:
			return 0, fmt.Errorf("proto: illegal wireType %d", wireType)
		}
		if iNdEx < 0 {
			return 0, ErrInvalidLengthVerifiedClientId
		}
		if depth == 0 {
			return iNdEx, nil
		}
	}
	return 0, io.ErrUnexpectedEOF
}

var (
	ErrInvalidLengthVerifiedClientId        = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowVerifiedClientId          = fmt.Errorf("proto: integer overflow")
	ErrUnexpectedEndOfGroupVerifiedClientId = fmt.Errorf("proto: unexpected end of group")
)
